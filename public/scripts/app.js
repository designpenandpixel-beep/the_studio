...
async function falImg(prompt){
  ...
  for(let i=0;i<120;i++){await sleep(2200);if(S.stopSb)throw new Error('Stopped');const rs=await falFetch(`https://queue.fal.run/${model}/requests/${d.request_id}/status`,{method:'POST',headers:{'Authorization':`Key ${k}`,'Content-Type':'application/json'}});const ds=await rs.json();if(ds.status==='COMPLETED'){const rr=await falFetch(`https://queue.fal.run/${model}/requests/${d.request_id}`,{method:'POST',headers:{'Authorization':`Key ${k}`,'Content-Type':'application/json'}});const rd=await rr.json();const u=rd.images?.[0]?.url;if(!u)throw new Error('No image URL');return u}if(ds.status==='FAILED')throw new Error('fal generation failed');}throw new Error('Timeout');
}
...
async function falVid(imgUrl,prompt,dur,ratio){
  ...
  if(!r.ok){const t=await r.text();throw new Error(`fal ${r.status}: ${t.substring(0,80)}`)}
  const d=await r.json();if(!d.request_id)throw new Error('No request_id');
  for(let i=0;i<120;i++){if(S.stopVid)throw new Error('Stopped');await sleep(3000);const rs=await falFetch(`https://queue.fal.run/${S.vidModel.id}/requests/${d.request_id}/status`,{method:'POST',headers:{'Authorization':`Key ${k}`,'Content-Type':'application/json'}});const ds=await rs.json();if(ds.status==='COMPLETED'){const rr=await falFetch(`https://queue.fal.run/${S.vidModel.id}/requests/${d.request_id}`,{method:'POST',headers:{'Authorization':`Key ${k}`,'Content-Type':'application/json'}});const rd=await rr.json();const u=rd.video?.url||rd.videos?.[0]?.url;if(!u)throw new Error('No video URL');return u}if(ds.status==='FAILED')throw new Error('Video generation failed');}throw new Error('Timeout');
}
